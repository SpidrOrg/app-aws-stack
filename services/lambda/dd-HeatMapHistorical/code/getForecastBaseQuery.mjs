...
const getJdaActualSubQuery = (asOnDate, customerP, categoryP, valueOrQuantity, lag, yago, viewType) => {
  const aStartDate = dfns.format(dfns.add(asOnDate, {months: lag, years: yago ? -1 : 0}), DB_DATE_FORMAT);
  const aLastDate = dfns.format(dfns.add(asOnDate, {months: lag + 2, years: yago ? -1 : 0}), DB_DATE_FORMAT);
  ...
  return `
      SELECT    Sum("${aggregateColumnName}") AS ${yago ? 'yagoActual': 'actual'}
      FROM      client_actual
...
export default function(refreshDateP, customer, category, valueOrQuantity, lag, forecastPeriodType = VIEW_TYPES.R3M){
  ...
  const QUERY = `
    ...
    SELECT  ROUND(((forecast / yagoActual - 1) * 100), 2), ROUND(((actual / yagoActual - 1) * 100), 2)
    FROM (
        ${getClientForecastSubQuery(refreshDateP, customerP, categoryP, valueOrQuantity, lag, forecastPeriodType)}
    ), (
        ${getJdaActualSubQuery(refreshDateP, customerP, categoryP, valueOrQuantity, lag, true, forecastPeriodType)}
    ), (
        ${getJdaActualSubQuery(refreshDateP, customerP, categoryP, valueOrQuantity, lag, false, forecastPeriodType)}
    )
  `
